...
test("金额始终在 1_000_000n 到 2_000_000n 之间", () => {
  for (let marketId = 1n; marketId <= 50n; marketId++) {
    for (let walletIndex = 0; walletIndex < 6; walletIndex++) {
      for (const side of ["yes", "no"] as const) {
        ...
        assert.ok(
          amount >= 1_000_000n && amount <= 2_000_000n,
...
test("返回值始终保持 6 decimals", () => {
  for (let marketId = 1n; marketId <= 50n; marketId++) {
    for (let walletIndex = 0; walletIndex < 6; walletIndex++) {
      for (const side of ["yes", "no"] as const) {
        const amount = seedAmount(marketId, walletIndex, side);
        assert.equal(amount % 1_000_000n, 0n, `金额不是 6 decimals: ${amount.toString()}`);
      }
    }
  }
});

test("YES/NO 对同一 marketId 与 walletIndex 在多次采样里至少有 15 次不同", () => {
  let diffCount = 0;
  ...
  assert.ok(diffCount >= 15, `YES/NO 差异次数不足: ${diffCount}`);
});
...
test("不同 walletIndex 在多次采样里至少有 15 次出现差异", () => {
  let diffCount = 0;
  ...
  assert.ok(diffCount >= 15, `walletIndex 差异次数不足: ${diffCount}`);
});
