...
test("金额落在 1-2 USDC（6 decimals）", () => {
  for (let m = 0n; m < 50n; m++) {
    for (let w = 0; w < 5; w++) {
      for (const side of ["yes", "no"] as const) {
        const a = seedAmount(m, w, side);
        assert.ok(a >= 1_000_000n && a <= 2_000_000n, `m=${m} w=${w} side=${side} a=${a}`);
      }
    }
  }
});

test("返回值始终保持 6 decimals", () => {
  for (let m = 0n; m < 50n; m++) {
    for (let w = 0; w < 5; w++) {
      for (const side of ["yes", "no"] as const) {
        const a = seedAmount(m, w, side);
        assert.equal(a % 1_000_000n, 0n);
      }
    }
  }
});
...
test("YES 与 NO 在同一 (marketId, walletIndex) 上独立（多次累计出现差异）", () => {
  ...
  assert.ok(differCount >= 15, "50 个样本至少 15 个 YES/NO 不等");
});
...
export function seedAmount(marketId: bigint, walletIndex: number, side: "yes" | "no"): bigint {
  ...
  const rand = mulberry32(seed);
  const usdc = 1 + Math.floor(rand() * 2);   // 1..2
...
- [ ] **Step 5：跑 generate-seeds（生成钱包池 + web/lib/seed-wallets.ts）**
  ...
  - 验证：`npm run topup` 应输出 12 个 healthy，0 needsTopup（即每个钱包余额至少 10 USDC，且与 1–2 USDC 单边 seed 的冷启动预算兼容）
...
- 单边 1–2 USDC 的冷启动预算应能让首轮 26 市场 seed 跑完，不必依赖旧的高 buffer 假设
