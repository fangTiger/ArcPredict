# ArcPredict MVP Implementation Plan
...
```solidity
    function test_Claim_PreventsReentrancy() public {
        ...
        // 设置 USDC transfer 给 alice 时回调 claim
        usdc.setReentrancyCallback(alice, abi.encodeWithSignature("claim(uint256)", id));
        // 让 alice 是恶意合约（通过 vm.etch 替换）—— 此处简化：直接 prank+revert 预期
        vm.prank(alice);
        // 第二次 claim 因 AlreadyClaimed 而 revert，被 reentrancy callback 触发的 require failed
        vm.expectRevert();                                       // require(ok, "MockUSDC: reentrancy call failed")
        market.claim(id);
    }
```
...
contract PredictionMarketInvariantTest is Test {
    ...
    /// 资金守恒：合约 USDC 余额 ≥ 所有未 claim 的应付
    function invariant_FundsConservation() public view {
        uint256 contractBal = usdc.balanceOf(address(market));
        uint256 owed = 0;
        for (uint256 id = 0; id < market.marketCount(); id++) {
            PredictionMarket.Market memory m = market.getMarket(id);
            if (m.outcome == PredictionMarket.Outcome.Unresolved) {
                owed += m.yesPool + m.noPool;
            } else if (m.outcome != PredictionMarket.Outcome.Invalid) {
                // 已结算：合约还应保留 winnerPool（fee 已转出）
                owed += m.winnerPool;
            } else {
                // Invalid：用户可退回的总额 = yesPool + noPool
                owed += m.yesPool + m.noPool;
            }
        }
        // 已 claim 的 user payout 已离开合约 ⇒ contractBal == owed - paid
        // 简化为 contractBal ≥ 任意时刻应付 - 已支付，即 contractBal ≥ 0 显然成立
        // 真正的约束：contractBal + paid ≥ staked
        assertGe(contractBal + handler.totalPaid(), handler.totalStaked());
    }
}
```
...
contract Deploy is Script {
    function run() external {
        ...
        // 写 ABI
        string memory abi_ = vm.readFile("out/PredictionMarket.sol/PredictionMarket.json");
        vm.writeFile("../web/lib/abis/PredictionMarket.json", abi_);
    }
}
```
...
contract CreateMarket is Script {
    ...
    function scale(int64 human, int32 feedExpo) internal pure returns (int64) {
        require(feedExpo <= 0, "feedExpo must be <= 0");
        uint64 mul = uint64(10) ** uint64(uint32(-feedExpo));
        return int64(human * int64(mul));
    }
...
export function FaucetCard() {
  ...
  if (!isConnected || (bal as bigint) > 0n) return null;
  ...
  { row: DashboardRow; side: boolean; onClose: () => void })
{
  ...
  useEffect(() => {
    ...
    } else if (isSuccess && step === 'betting') {
      setStep('done');
            ...
            <div key={r.id.toString()} className="bg-elevated border border-white/[0.06] rounded-xl p-4">
              ...
              <div className="flex justify-between items-center">
                <div className="text-xs font-mono text-zinc-500">
                  {r.pendingPayout > 0n ? `Payout: ${fmtUsdc(r.pendingPayout)} USDC` : r.claimed_ ? 'Claimed' : 'No payout'}
                </div>
                {canClaim && (
...
npm init -y
npm i viem @pythnetwork/hermes-client dotenv
```

- [ ] **Step 2: ResolveDueMarkets.ts**
...
const MKT      = process.env.PREDICTION_MARKET! as `0x${string}`;
...
const abi = JSON.parse(readFileSync('../../out/PredictionMarket.sol/PredictionMarket.json', 'utf8')).abi;
...
const wc = createWalletClient({ account, chain: arcTestnet as any, transport: http(RPC_URL) });
const pc = createPublicClient({ chain: arcTestnet as any, transport: http(RPC_URL) });
...
async function main() {
  ...
  for (let i = 0n; i < count; i++) {
    ...
    if (nowSec > Number(m.resolveAfter) + 300) {
      console.log(`market ${i}: 窗口已过 (resolveAfter=${m.resolveAfter}), 跳过`);
      continue;
    }
    try {
      const res = await hermes.getPriceUpdatesAtTimestamp(Number(m.resolveAfter), [m.pythPriceId], {
        encoding: 'hex', parsed: false,
      });
      const updateData = res.binary.data.map((s: string) => (`0x${s}`) as `0x${string}`);

      const fee = await pc.readContract({
        address: '0x...' as `0x${string}`,                       // TODO Pyth address
        abi: parseAbi(['function getUpdateFee(bytes[]) view returns (uint256)']),
      ...
      console.log(`market ${i}: 已结算 tx=${hash}`);
    } catch (e) {
      console.error(`market ${i}: 失败`, e);
    }
  }
}
...
```cron
*/1 * * * * cd /path/to/ArcPredict/contracts/script/ops && /usr/local/bin/node ResolveDueMarkets.ts >> /var/log/arc-predict-resolve.log 2>&1
```
