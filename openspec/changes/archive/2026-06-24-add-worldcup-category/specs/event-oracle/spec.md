...
- **AND** 实现 SHALL NOT 要求任何"预注册"或"初始化"步骤；`Pending` 作为枚举默认值通过 Solidity mapping 零值天然成立

### Requirement: AdminEventOracle MVP 实现

系统 SHALL 提供 `AdminEventOracle` 合约作为 `IEventOracle` 的 MVP 实现，采用"owner 提交 + 72h 异议期 + 100 USDC 质押挑战 + owner 简单裁定"模型。**复用现有 `PredictionMarket` 的 `Ownable` 单 EOA owner 模式**，沿用同一 `OWNER_PRIVATE_KEY` 配置。
...
- **AND** 挑战方 SHALL 收回 100 USDC 质押金 + 100 USDC bonus（bonus 由协议 feeRecipient 预留账户支付，部署时配置预留额度）
- **AND** owner 可以提交新的 `proposeResult`，72h 异议期重新计时

#### Scenario: owner 驳回挑战
- **WHEN** owner 调用 `confirmProposal(eventId)`（坚持原提案）
- **THEN** 事件状态 SHALL 变为 `Finalized`
- **AND** 挑战方质押的 100 USDC SHALL 转入协议 feeRecipient（没收作为协议收入）

#### Scenario: owner 未响应挑战
- **WHEN** 距离首次提案的 72h 异议期已结束，但事件仍处于 `Challenged` 状态（owner 既未 revoke 也未 confirm）
