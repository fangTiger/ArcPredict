...
describe('lens-preloader', () => {
  it('warm invokes the lens route handler and reports success', async () => {
    const warm = vi.fn().mockResolvedValue({ status: 'ok' });
    const preloader = createLensPreloader({ warmFn: warm });

    await expect(preloader.warm({
      eventId: '0xab' as any,
      category: 'macro',
      question: 'Q',
  ...
  it('returns a warning for warmFn failure without throwing', async () => {
    ...
    await expect(
      ...
      status: 'warning',
      warning: expect.stringContaining('Lens preload'),
    });
  });

  it('surfaces a warning when warmFn returns an error status', async () => {
    const warm = vi.fn().mockResolvedValue({ status: 'error' });
    const preloader = createLensPreloader({ warmFn: warm });
